...
class SmartEV(EV):
    def __init__(self, name, trips, battery_capacity, user_type, oracle=False):
    ...
    def smart_charging(self, place, arrival_time, departure_time, Epredicted, next_destination):
        ...
        if place in ["home", "workplace"]:
            ...
            if self.trips.iloc[-1]['Ebattery'] < Eneeded:
                if surplus > 0:
                    if surplus >= Eneeded:
                        # charge the car until SoC hLimit is reached
                    ...
                    self.next_destination_check(arrival_time, departure_time, place_power, place, next_destination, Eneeded)
            else:
                self.charge_EV_REC(arrival_time, departure_time, place_power, place, rec_predictions, next_destination, Eneeded)
    ...
    def discharge_to_REC(self, current_time, plug_out_time, Eneeded, power_place, place, new_row):
        ...
        if to_REC > 0:
            ...
            new_row["Eneeded"] = Eneeded
            self.SoC -= to_REC / self.battery_capacity
            new_row['SoC'] = self.SoC
        else:
