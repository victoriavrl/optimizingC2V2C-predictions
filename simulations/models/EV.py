...
class EV:
    ...
    columns = ["datetime", "state", "consumption", "SoC", "Ebattery", "EchargedBattery",
    ...
    def __init__(self, name, inputs, battery_capacity, public=True):
        ...
        self.SoC = 0.5  # in kWh (initially 50%)
        self.to_workplace = pd.read_csv('data_REC/workplace/data.csv', parse_dates=['datetime']).copy()
        self.to_workplace['from_ev'] = 0
        self.to_home = pd.read_csv('data_REC/workplace/data.csv', parse_dates=['datetime']).copy()
        self.to_home['from_ev'] = 0

    def battery_is_full(self, current_time, plug_out_time, place):
        if self.trips.iloc[-1]["SoC"] >= EV.SoChLimit:
    ...
    def charge_EV(self, plug_in_time, plug_out_time, place):
        ...
        if place in ["home", "workplace"]:
            ...
            while current_time < plug_out_time and self.trips.iloc[-1]["SoC"] < EV.SoChLimit:
                new_row = self.create_row(current_time, plug_out_time, place)
            ...
            while current_time < plug_out_time and self.trips.iloc[-1]["SoC"] < EV.SoChLimit:
                new_row = self.create_row(current_time, plug_out_time, place)
                new_row = self.charge_from_grid(new_row, get_place_power(place)*0.25)
                self.update_trips(new_row)
                current_time += pd.Timedelta("15min")
        else:
    ...
    def apply_energy(self, new_row, energy_value, source):
        ...
        new_row["SoC"] = new_row["Ebattery"] / self.battery_capacity
        self.SoC = new_row["SoC"]
    ...
    def create_row(self, time, plug_out_time, place):
        ...
        return {
            "datetime": time, "state": place, "consumption": 0,
            "SoC": last["SoC"], "Ebattery": last["Ebattery"],
            "EchargedBattery": 0, "EbattR": last["EbattR"],
