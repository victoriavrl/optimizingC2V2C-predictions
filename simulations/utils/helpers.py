...
def compute_Eavailable(REC_predictions):
    ...
    if "power" in REC_predictions.columns:
        # Compute energy from power via trapezoidal rule
        REC_predictions["delta_t"] = REC_predictions["datetime"].diff().dt.total_seconds() / 3600
        REC_predictions["avg_power"] = (REC_predictions["power"] + REC_predictions["power"].shift(-1)) / 2
        valid = REC_predictions["avg_power"] > 0
        REC = np.nansum((REC_predictions["avg_power"] * REC_predictions["delta_t"]).where(valid).iloc[:-1])
        no_surplus_moments = REC_predictions.loc[REC_predictions["power"] <= 0, "datetime"].values

    elif "energy" in REC_predictions.columns:
        ...
        raise ValueError("REC_predictions must contain either a 'power' or 'energy' column.")
...
def compute_Eneeded(Epredicted, EbattCap):
    ...
    anxiety = 1.5
    margin = Epredicted / EbattCap * anxiety
    Eunpredicted = 0.2 * EbattCap + margin
    return Epredicted + Eunpredicted
